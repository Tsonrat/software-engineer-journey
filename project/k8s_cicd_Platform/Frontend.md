...
<img width="1907" height="887" alt="platform-overview" src="https://github.com/user-attachments/assets/5564495a-6dac-4244-884f-92a4505a318c" />
*Platform Overview｜管理首頁整合 Project、Deployment、Image Management、Artifact Push Task、Template 與 Registry 等主要功能入口。*
...
<img width="606" height="504" alt="Login" src="https://github.com/user-attachments/assets/9261e3b6-d33f-40a7-ba1b-5e1f7b94025c" />

*Authentication Entry｜使用者透過統一的 Identity Provider 登入管理平台。*
...
<img width="1693" height="518" alt="Artifact Explorer" src="https://github.com/user-attachments/assets/35213519-ecb0-4742-855a-4c4a0467dd18" />
*Artifact Version and Platform Metadata｜依 Artifact Tag 顯示版本狀態、更新時間與支援的 OCI Platform。*


### Artifact Creation and Platform Resolution｜Artifact 建立與平台解析

使用者從 Registry Explorer 選擇 Image Tag 後，可以建立平台管理的 Artifact。前端會請求後端解析該 Tag 的 OCI Manifest，取得可用的 Platform 與對應 Digest，再由使用者選擇需要納入管理的版本。

此流程包含：

- 解析選定 Image Tag 的 OCI Manifest。
- 顯示不同 Architecture 與 Variant。
- 顯示各 Platform 對應的 Digest。
- 選擇需要管理或同步的 Platform。
- 將符合條件的 Artifact 標記為 Dockerfile Base Image。
- 建立後續 Artifact Management 與 Registry Synchronization 所需資料。

<img width="591" height="701" alt="create_atifact" src="https://github.com/user-attachments/assets/158438d6-7dff-4492-8a0e-33a00a389978" />
...
<img width="925" height="362" alt="Select Docker Base Image" src="https://github.com/user-attachments/assets/be699caa-5fe3-46aa-b174-1aefb348c733" />
*Base Image Selector｜使用者可以透過搜尋、Registry Filter、Version Status 與 Pagination，選擇平台管理且狀態有效的 Docker Base Image。*
...
<img width="975" height="619" alt="selected-project-config" src="https://github.com/user-attachments/assets/59b1768f-4064-4adc-906e-65c208ae8b95" />
*Project Deployment Configuration｜平台依 DEV、UAT、PROD 隔離部署設定，並顯示 Helm、Dockerfile 與 Shell Template 的選擇狀態及環境完整度。*

The environment summary presents the readiness of Helm, Dockerfile, and Shell configurations before the user initiates deployment package generation and the subsequent delivery workflow.

環境摘要會呈現 Helm、Dockerfile 與 Shell Configuration 的完整度，讓使用者在產生 Deployment Package 並進入後續交付流程前，先確認必要設定是否準備完成。
...
<img width="1717" height="817" alt="helm" src="https://github.com/user-attachments/assets/7bdd3d0b-5bd4-4567-8ec9-c3d538cec159" />
*Helm Template Configuration｜使用者可以編輯 Environment-specific Values、選擇 ConfigMap／Secret、比較 Template Default Values，並預覽 Rendered Helm Manifest 與 Validation Result。*
...
<img width="988" height="720" alt="deployment_log" src="https://github.com/user-attachments/assets/b9b719f4-5fc4-40ff-82e7-46383f0bbb36" />
*Deployment History｜前端呈現 Deployment Package 的執行環境、狀態、開始與完成時間，並在成功後提供結果下載。*
...
<img width="992" height="172" alt="push_task_pending" src="https://github.com/user-attachments/assets/145b149a-a198-44fc-b035-fd70b93950ab" />
<img width="992" height="174" alt="push_task_fail" src="https://github.com/user-attachments/assets/6ed2e134-5939-43a4-9a30-5aebedb9e768" />
<img width="992" height="178" alt="push_task_running" src="https://github.com/user-attachments/assets/f5c670d3-8917-4aa7-a756-d9ec7c17870a" />

### Persistent Failure Detail｜持久化失敗紀錄

Artifact Push Task 執行失敗時，前端除了顯示 `FAILED` Status，也會呈現 Registry、Artifact、Tag、Platform、執行時間與 Failure Reason，讓管理者能夠判斷失敗發生在哪一個 Registry Operation。

Task Execution 與 Push Log 分開管理：Task 用於呈現目前工作狀態與 Retry 操作，Push Log 則保存完成或失敗後的執行結果及診斷資訊。

<img width="751" height="706" alt="push_log_error" src="https://github.com/user-attachments/assets/0948796e-df43-42be-8e68-a9106ade7f26" />

*Artifact Push Failure Detail｜Persistent Push Log 保存 Artifact、Tag、Platform、執行時間與上游 Registry 回傳的 Failure Reason，協助管理者分析 OCI Artifact Synchronization Failure。*
...
Asynchronous Task Failure 會保留於 Persistent Task Log，並在 Detail View 中呈現 Status、Failure Reason、Registry Context 與 Platform Metadata；相關畫面請參考 Artifact Push Task 章節。

非同步任務失敗時，系統會將結果保存於 Persistent Task Log，並在 Detail View 顯示狀態、失敗原因、Registry Context 與 Platform Metadata。
